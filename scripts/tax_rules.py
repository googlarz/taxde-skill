"""
Shared TaxDE tax rules and helpers.

The figures below are intentionally limited to the years the skill currently
claims to support. Where an official 2026 social-insurance-linked ceiling was
not available in the bundled primary sources, the helper returns a note so the
caller can degrade confidence instead of silently overstating certainty.
"""
...
def resolve_supported_year(year: int) -> tuple[int, Optional[str]]:
    ...
    if year < supported_years[0]:
        return supported_years[0], (
            f"Tax year {year} is older than the bundled rules. "
            f"Using {supported_years[0]} figures as a fallback."
        )

    latest = supported_years[-1]
    return latest, (
        f"Tax year {year} is newer than the bundled rules. "
        f"Using {latest} figures as a fallback."
    )
...
def equipment_useful_life(description: str) -> int:
    ...
    if any(token in desc for token in ("laptop", "computer", "notebook", "macbook")):
        return 3
    if any(token in desc for token in ("monitor", "bildschirm", "display")):
        return 3
    if any(token in desc for token in ("drucker", "printer", "scanner")):
        return 3
    if any(token in desc for token in ("handy", "smartphone", "telefon", "phone")):
        return 3
    if any(token in desc for token in ("schreibtisch", "desk", "stuhl", "chair")):
        return 13
    if any(token in desc for token in ("kamera", "camera")):
        return 7
    return 3


def calculate_equipment_deduction(
    amount: float,
    description: str,
